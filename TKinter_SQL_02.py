...
def read():
    ...
    conn = connection()
    cursor = conn.cursor()
    sql=f"SELECT item_id, name, price, quantity, category, date FROM dbo.python_stocks ORDER BY id DESC"
    cursor.execute(sql)
...
def save():
    ...
    try:
        conn = connection()
        cursor = conn.cursor()
        sql=f"SELECT * FROM dbo.python_stocks WHERE item_id = '{itemId}' "
        cursor.execute(sql)
        ...
        if len(checkItemNo) > 0:
            ...
            conn = connection()
            cursor = conn.cursor()
            sql=f"INSERT INTO dbo.python_stocks (item_id, name, price, quantity, category) VALUES ('{itemId}','{name}','{price}','{qnt}','{cat}')"
            cursor.execute(sql)
        conn.commit()
        conn.close()
...
def update():
    ...
    try:
        conn = connection()
        cursor = conn.cursor()
        sql=f"UPDATE dbo.python_stocks SET name = '{name}', price = '{price}', quantity = '{qnt}', category = '{cat}' WHERE item_id = '{itemId}' "
        cursor.execute(sql)
        conn.commit()
        conn.close()
        for num in range(0,5):
        ...
        if(my_tree.selection()[0]):
            ...
            if(decision != 'yes'):
                ...
                try:
                    conn = connection()
                    cursor = conn.cursor()
                    sql=f"DELETE FROM dbo.python_stocks WHERE item_id = '{itemId}' "
                    cursor.execute(sql)
                    conn.commit()
                    conn.close()
...
def find():
    ...
    if(itemId and itemId.strip()):
        sql = f"SELECT item_id, name, price, quantity, category, date FROM dbo.python_stocks WHERE item_id LIKE '%{itemId}%' "
    elif(name and name.strip()):
        sql = f"SELECT item_id, name, price, quantity, category, date FROM dbo.python_stocks WHERE name LIKE '%{name}%' "
    elif(price and price.strip()):
        sql = f"SELECT item_id, name, price, quantity, category, date FROM dbo.python_stocks WHERE price LIKE '%{price}%' "
    elif(qnt and qnt.strip()):
        sql = f"SELECT item_id, name, price, quantity, category, date FROM dbo.python_stocks WHERE quantity LIKE '%{qnt}%' "
    elif(cat and cat.strip()):
        sql = f"SELECT item_id, name, price, quantity, category, date FROM dbo.python_stocks WHERE category LIKE '%{cat}%' "
    else:
