...
def read():
    conn = connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM dbo.Python_students")
...
def add():
    ...
    if (studid == "" or studid == " ") or (fname == "" or fname == " ") or (lname == "" or lname == " ") or (address == "" or address == " ") or (phone == "" or phone == " "):
        ...
        try:
            conn = connection()
            cursor = conn.cursor()
            cursor.execute("INSERT INTO dbo.Python_students VALUES ('"+studid+"','"+fname+"','"+lname+"','"+address+"','"+phone+"') ")
            conn.commit()
            conn.close()
        except:
...
def reset():
    ...
    if decision != "yes":
        ...
        try:
            conn = connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM dbo.Python_students")
            conn.commit()
            conn.close()
        except:
...
def delete():
    ...
    if decision != "yes":
        ...
        try:
            conn = connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM dbo.Python_students WHERE STUDID='"+str(deleteData)+"'")
            conn.commit()
            conn.close()
        except:
            messagebox.showinfo("Error", "Sorry an error occured")
...
def search():
    ...
    conn = connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM dbo.Python_students WHERE STUDID='"+
    studid+"' or FNAME='"+
    fname+"' or LNAME='"+
    lname+"' or ADDRESS='"+
    address+"' or PHONE='"+
...
def update():
    ...
    if (studid == "" or studid == " ") or (fname == "" or fname == " ") or (lname == "" or lname == " ") or (address == "" or address == " ") or (phone == "" or phone == " "):
        ...
        try:
            conn = connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE dbo.Python_students SET STUDID='"+
            studid+"', FNAME='"+
            fname+"', LNAME='"+
            lname+"', ADDRESS='"+
            address+"', PHONE='"+
